...
public class FunctionalTest {
    ...
    public void redirectHomePage() {
        running(fakeApplication(), new Runnable() {
           public void run() {
               Result result = callAction(DeliveryController.index());
    ...
    public void listComputersOnTheFirstPage() {
        running(fakeApplication(), new Runnable() {
           public void run() {
               Result result = callAction(DeliveryController.list(0, "name", "asc", ""));
    ...
    public void filterComputerByName() {
        running(fakeApplication(), new Runnable() {
           public void run() {
               Result result = callAction(DeliveryController.list(0, "name", "asc", "Apple"));
    ...
    public void createANewComputer() {
        running(fakeApplication(), new Runnable() {
            public void run() {
                Result result = callAction(DeliveryController.save());
                ...
                assertThat(status(result)).isEqualTo(SEE_OTHER);
                assertThat(redirectLocation(result)).isEqualTo("/computers");
                assertThat(flash(result).get("success")).isEqualTo("Delivery FooBar has been created");
                
                result = callAction(DeliveryController.list(0, "name", "asc", "FooBar"));
                assertThat(status(result)).isEqualTo(OK);
